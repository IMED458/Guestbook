              ...
              {editingTheme && (
                <div className="p-6 rounded-2xl bg-white border border-stone-200 space-y-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2">
                      <div className="flex items-center justify-between mb-2">
                        <label className="block text-xs font-bold text-stone-800">
                          {lang === 'ka' ? 'შრიფტი და ხელნაწერის სტილი' : 'Font Family & Handwriting Style'}
                        </label>
                        <span className="text-[11px] text-rose-700 bg-rose-50 border border-rose-200 px-2.5 py-0.5 rounded-full font-semibold">
                          {lang === 'ka' ? 'ქართული შრიფტები • ხელნაწერი • კალიგრაფია' : 'Georgian Fonts • Cursive • Calligraphy'}
                        </span>
                      </div>
                      <FontPicker
                        selectedFont={editingTheme.fontStyle}
                        onChange={(newFont) =>
                          setEditingTheme({
                            ...editingTheme,
                            fontStyle: newFont
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-stone-700 mb-1">
                        {lang === 'ka' ? 'ღილაკის სტილი' : 'Button Style'}
                      </label>
                      <select
                        ...
                        className="w-full px-3 py-2.5 bg-stone-50 border border-stone-300 rounded-xl text-xs focus:outline-none"
                      >
                        <option value="pill">Pill ({lang === 'ka' ? 'მომრგვალებული ოვალი' : 'Rounded Pill'})</option>
                        <option value="rounded">Rounded ({lang === 'ka' ? 'სტანდარტული კუთხეები' : 'Standard Rounded'})</option>
                        <option value="minimal">Minimal ({lang === 'ka' ? 'სადა მინიმალისტური' : 'Minimal Clean'})</option>
                      </select>
                    </div>
                  </div>

                  {/* Live Preview Card in Admin Design Tab */}
                  <div className="mt-4 p-5 rounded-2xl border border-stone-200 bg-stone-50/70 space-y-2">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-stone-500 block">
                      {lang === 'ka' ? 'შერჩეული დიზაინის ცოცხალი გადახედვა:' : 'Live Preview of Selected Design:'}
                    </span>
                    <div
                      className="p-6 rounded-2xl border transition-all shadow-sm"
                      style={{
                        backgroundColor: editingTheme.bgColor,
                        fontFamily: getFontFamily(editingTheme.fontStyle)
                      }}
                    >
                      <div
                      ...
                      >
                        {currentBook?.hostNames || currentBook?.title || 'ნიკა და ანა'}
                      </div>
                      <div className="text-xl font-normal text-stone-900 leading-relaxed mb-3">
                        &ldquo;{currentBook?.welcomeMessage || 'მოგესალმებით ჩვენს საქორწილო სტუმრების წიგნში ❤️'}&rdquo;
                      </div>
                      <div className="p-3.5 rounded-xl bg-white/80 border border-stone-200/80 text-xs text-stone-800 space-y-1">
                        <div className="font-semibold text-stone-900">
                          გიორგი და ნინო • {lang === 'ka' ? 'სტუმრის მილოცვა' : 'Guest Message'}
                        </div>
                        <p className="text-stone-700 italic">
                          {lang === 'ka'
                            ? 'გილოცავთ ამ ულამაზეს დღეს! გისურვებთ ულევ სიყვარულს, ხვავსა და ულამაზეს საერთო გზას! ❤️🥂'
                            : 'Wishing you both a lifetime of unending love, joy, and unforgettable moments together! ❤️🥂'}
                        </p>
                      </div>
                    </div>
                  </div>
